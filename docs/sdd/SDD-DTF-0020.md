...
**Verificador independente:** sim — segunda rodada, subagente separado tanto da sessão implementadora quanto da primeira verificação, sem ler o histórico de nenhuma delas; entrada foi esta SDD e o diff `9d98af0..de1c893` (PR #61, commits `0185688`, `d119055`). Re-verificação em 2026-09-15 em `main` (HEAD `843599a`, código de RF01–RF06 idêntico ao de `de1c893`; o descompasso nº 1 registrado na verificação anterior — falso positivo de `check_hooks.py` regra 3 em comando shell-form — já foi corrigido por SDD-DTF-0027/0029, presentes em `main`). Critério 9 fechado em 2026-09-15 em sessão real do usuário (não subagente): SessionStart e PostToolUse dispararam exatamente como especificado (linha 9 da tabela acima). Status avança para `implemented`. Relatório complementar e sensores detalhados em `docs/sdd/validation.md`.

| # | Comando rodado | Saída (resumo) | Sensor | Passou? |
|---|---|---|---|---|
| 1 | `python3 -m pytest _framework/scripts/tests/test_hook_session_start.py -v` | `8 passed in 1.31s`, exit 0 | Mutação: `find_handoffs` com `docs/*/HANDOFF.md` antes da raiz → `test_handoff_raiz_e_docs_raiz_primeiro` falha (`docs/EVM/HANDOFF.md, HANDOFF.md` em vez de `HANDOFF.md, docs/EVM/HANDOFF.md`); restaurado via `git checkout --` → `8 passed` de novo | sim |
| 2 | `python3 -m pytest _framework/scripts/tests/test_hook_post_edit.py -v` | `8 passed in 1.63s`, exit 0 | Mutação: `problems_for` deixa de somar `validate_state.check_sdd` para `type == "SDD"` → `test_sdd_implemented_com_evidencia_vazia_bloqueia` falha (`0 == 2`); restaurado → `8 passed` | sim |
| 3 | `python3 -m pytest _framework/scripts/tests/test_render_prompts_mechanization.py -v` | `12 passed in 0.21s`, exit 0 | Mutação: `if "prompt" in mech` vira `if False and "prompt" in mech` em `validate_mechanizations` → `test_validate_mechanizations_rejeita_prompt_em_hook` falha (`DID NOT RAISE SystemExit`); restaurado → `12 passed` | sim |
| 4 | `python3 -m pytest _framework/scripts/tests/test_check_hooks.py -v` | `12 passed in 0.56s`, exit 0 | Mutação: checagem de prefixo `${CLAUDE_PROJECT_DIR}/` em `check_settings` desligada (`if False and ...`) → 3 testes falham (`test_command_shell_sem_referencia_reprova`, `test_command_shell_prefixo_falso_reprova`, `test_caminho_relativo`); restaurado → `12 passed` | sim |
| 5 | `python3 -c "import json; s=json.load(open('.claude/settings.json'))['hooks']; …"` (comando da tabela, literal) | `['PostToolUse', 'PreToolUse', 'SessionStart'] {'command'} True`, exit 0 | coberto pelo sensor do critério 3 (gerador recusa `prompt`/caminho relativo) | sim |
| 6 | `python3 _framework/scripts/check_hooks.py && python3 _framework/scripts/framework_check.py --auto` | `✅ .claude/settings.json: hooks ok.`; `-- hooks do harness`; `✅ Todas as verificações do framework passaram.`, exit 0 | Mutação: `if settings.exists()` vira `if False` em `framework_check.py` → `--auto` some com a linha `-- hooks do harness` (`grep -c` = 0, exit 1); restaurado → linha volta e exit 0 | sim |
| 7 | bloco C7 literal (script em arquivo, rodado com `bash`) | 1ª: stderr `framework: /tmp/tmp.VzZLrHRDIv/sdd/SDD-DTF-0017.md viola gate(s)` + `SDD-DTF-0017: 'Evidência de verificação' está vazia e o status é implemented`, `exit=2`; 2ª: sem saída, `exit=0` | o próprio bloco discrimina (arquivo mutado vs. original); reforçado pelos sensores dos critérios 2 e 4 (`return 2`→`return 0` e `check_sdd` pulado já provados acima) | sim |
| 8 | `ruff check _framework/scripts && ruff format --check _framework/scripts && mypy _framework/scripts && python3 -m pytest && python3 _framework/scripts/render_prompts.py --check && python3 _framework/scripts/check_renderings.py` | `All checks passed!`; `21 files already formatted`; `Success: no issues found in 21 source files`; `81 passed in 3.51s`; `render --check` exit 0 (cópias `sincronizado`); `✅ 5 renderização(ões) concordam` (2 avisos pré-existentes sobre `PRD`/`TS` legados no cursor), todos exit 0 | regressão e estático, sem sensor dedicado | sim |
| 9 | sessão real do harness (repl não-interativo, hooks reais ativos): 1ª interação do usuário dispara SessionStart antes de qualquer mensagem; depois, `Edit` esvaziando a tabela "Evidência de verificação" de uma cópia descartável de `docs/sdd/SDD-DTF-0017.md` | SessionStart, surfaced ao modelo antes da 1ª resposta: `HANDOFF.md encontrado em: HANDOFF.md. Antes de qualquer outra coisa, use a skill pickup: confirme o status real dos ids citados e releia do disco os arquivos que vai alterar.` — resposta do modelo já abriu citando a skill `pickup`/`HANDOFF.md`. Depois, `PostToolUse:Edit hook blocking error` com stderr `framework: /home/michel/doc-traceability-framework/docs/sdd/SDD-DTF-0017-copia.md viola gate(s) — corrija antes de seguir:\n  - SDD-DTF-0017: 'Evidência de verificação' está vazia e o status é \`implemented\` — checklist de memória não satisfaz o gate.` — modelo relatou o stderr ao usuário. Cópia apagada sem commit; `git status --short` limpo (só `.claude/worktrees/` e `HANDOFF.md` pré-existentes, alheios). | sem sensor — critério manual | sim |

Todas as mutações de sensor foram feitas diretamente nos arquivos do repositório (nunca `git stash`, nunca commit) e restauradas com `git checkout -- <arquivo>`; `git status --short` ficou limpo (só `.claude/worktrees/` não rastreado, pré-existente e alheio a esta verificação) ao final de cada uma e ao final da sessão.
