...
# Verificação — SDD-DTF-0001

- **Veredito:** PASS
- **Diff verificado:** `193f1fe4b49a9b86d8512a066c806995cfa112eb..e13daacc6b4c6d1193b361452dafd66dfbc43b14` (parent mainline do merge commit `e13daac` — "Merge pull request #7 from .../sdd/SDD-DTF-0001-superficie-entrada" — antes de qualquer merge da própria mudança). A verificação de critérios foi rodada contra o `HEAD` atual (`843599a`), bem mais à frente, porque a SDD nunca chegou a `implemented` e o código evoluiu por cima em SDDs subsequentes (0002 a 0027+) — os critérios de aceite continuam válidos como comportamento atual, não só como snapshot do PR original.
- **Verificador independente:** sim — sessão nova, sem ler histórico da sessão que implementou (SDD criada e mergeada em 2026-08-29, mais de duas semanas antes desta verificação, `git log --all --grep "SDD-DTF-0001"` usado só para achar o SHA do merge, não para herdar raciocínio).

| Critério | Comando rodado | Saída (resumo) | Sensor | Passou? |
|---|---|---|---|---|
| 1. RF01 — `AGENTS.md` gerado e em dia | `python3 _framework/scripts/render_prompts.py && python3 _framework/scripts/render_prompts.py --check` | Todas as linhas `✅ ... em dia.` / `sincronizado.`, exit 0 nas duas | ver critério 8 (mesmo mecanismo) | Sim |
| 2. RF01 — limite de tamanho | `wc -l < AGENTS.md` | `119` (limite 120) | sem teste automatizado — medição direta | Sim |
| 3. RF01 — conteúdo do caminho comum | `grep -c -E "small\|medium\|SDD\|SPEC" AGENTS.md` | `19` (≥ 4 exigido) | sem teste automatizado — medição direta | Sim |
| 4. RF02 — `QUICKSTART.md` dentro do limite | `wc -l < QUICKSTART.md` | `57` (limite 80) | sem teste automatizado — medição direta | Sim |
| 5. RF02 — não cita tipo/status inexistente | `python3 _framework/scripts/check_renderings.py` | `✅ 5 renderização(ões) concordam com workflow-rules.yaml (8 tipos ativos, 6 Iron Laws, 4 níveis).` exit 0; 2 avisos pré-existentes e não bloqueantes sobre `prompts/cursor/doc-framework.mdc` citar PRD/TS sem marcar como legado (mesmo aviso já presente na evidência original de 2026-08-29) | mutação: `legacy_types = []` em `check_renderings.py` — os dois avisos de tipo legado desaparecem silenciosamente (regressão não detectada); restaurado via `git checkout --`, avisos voltam e exit permanece 0 | Sim |
| 6. RF03 — PRD e TS fora do schema | `python3 -c "import yaml;d=yaml.safe_load(open('_framework/rules/workflow-rules.yaml'))['document_types'];assert 'PRD' not in d and 'TS' not in d"` | sem saída, exit 0 | mutação: injetada chave `PRD: {}` sob `document_types:` — `AssertionError`, exit 1; restaurado via `git checkout --`, exit 0 de novo | Sim |
| 7. RF03 — projetos já mapeados não reprovam | `python3 _framework/scripts/framework_check.py --auto` | `✅ Todas as verificações do framework passaram.` (projetos em `examples/`, incluindo um sob framework 1.7.0 e outro sob 2.0.0 — versões anteriores à 2.1.0, exercitando a não-retroatividade) | sem sensor — comando é o próprio validador de regressão geral | Sim |
| 7b. RF03 (borda) — id legado segue reconhecido | `cd _framework/scripts && python3 -c "from framework_lib import ID_PATTERN; assert ID_PATTERN.search('PRD-EVM-0001') and ID_PATTERN.search('TS-EVM-0001')"` | sem saída, exit 0 | mutação: `_derive_constants` alterado para `types = active or _FALLBACK_DOC_TYPES` (remove a união com `legacy_document_types`) — `AssertionError`, exit 1; restaurado via `git checkout --`, exit 0 de novo | Sim |
| 8. RF07 (borda) — edição manual é barrada | `echo "linha intrusa" >> AGENTS.md && python3 _framework/scripts/render_prompts.py --check` | `❌ AGENTS.md: divergente do gerado — rode render_prompts.py.`, exit 1; demais alvos seguem `em dia`/`sincronizado` | **é o sensor**: edição manual introduzida de propósito nesta sessão, `--check` reprovou nomeando o arquivo; restaurado com `git checkout -- AGENTS.md`, `--check` volta a exit 0 | Sim |
| 9. RF11 — paridade entre repositórios | `diff AGENTS.md` e `diff QUICKSTART.md` entre `doc-traceability-central` (checkout local, `main`@`be8accc`) e este worktree | ambos sem saída (idênticos) — os dois artefatos desta SDD estão em paridade. `diff -rq --exclude=__pycache__` do `_framework/` completo mostra divergência ampla, mas é causada por SDDs posteriores (0006, 0009, 0011...) aplicadas só neste repositório e ainda não sincronizadas para `doc-traceability-central` — drift operacional do ciclo de sincronização, não regressão do escopo desta SDD | sem sensor — comparação direta de arquivo | Sim (para o escopo desta SDD) |
| 10. Determinismo | `render_prompts.py` rodado duas vezes seguidas, `git status --porcelain` comparado entre as duas execuções | ambas as execuções deixam a árvore de trabalho limpa (nenhuma diferença entre a primeira e a segunda rodada) | sem sensor — comparação direta de saída | Sim |

Checagem mecânica depois de preencher a evidência e mudar o status: `python3 _framework/scripts/validate_state.py docs/sdd/SDD-DTF-0001.md` → ver resultado na seção de conformidade abaixo.

## Conformidade requisito ↔ código (SDD-DTF-0001)

- RF01: `_framework/scripts/render_prompts.py` tem um formato `agents` (alvo `../AGENTS.md`) que monta o conteúdo a partir de `build_block(rules)` mais o texto de caminho `small`/`medium`. `AGENTS.md` gerado tem 119 linhas (≤ 120), cita os quatro termos exigidos e as Iron Laws do YAML, confirmado por `check_renderings.py`.
- RF02: mesmo script tem alvo `../QUICKSTART.md`, 57 linhas (≤ 80); `check_renderings.py` não acusa tipo/status inexistente nele (o script deliberadamente exclui `QUICKSTART.md` do laço de `RENDERINGS` por não ter Iron Laws/tabela de sizing completas — comentário explícito no código, não um buraco de cobertura).
- RF03: `_framework/rules/workflow-rules.yaml` tem `PRD` e `TS` só em `legacy_document_types` (linhas 355+), ausentes de `document_types` (linha 208+). `framework_lib._derive_constants` une `document_types` e `legacy_document_types` em `DOC_TYPES`/`ID_PATTERN`, preservando o reconhecimento de ids `PRD-*`/`TS-*` já emitidos. `check_renderings.py` lê `legacy_types` de `legacy_document_types` (não mais por `deprecated_since`).
- Casos de borda: adaptador ausente é criado por `render_prompts.py` (comportamento herdado, não teve regressão testada nesta verificação por não ser o foco desta SDD); documento já emitido com `type: PRD`/`TS` não reprova (`framework_check.py --auto` passou nos projetos de exemplo sob framework antigo); repositório sem `_framework/` não foi testado nesta sessão (não há esse tipo de projeto nos exemplos atuais) — não é um requisito com teste automatizado, e a SDD já registra isso no próprio texto ("RULE_SINCE não recebe entrada nesta etapa").
- Direção inversa: `git diff --stat 193f1fe4b49a9b86d8512a066c806995cfa112eb e13daac` lista `AGENTS.md`, `QUICKSTART.md`, `_framework/rules/workflow-rules.yaml`, `_framework/scripts/check_renderings.py`, `_framework/scripts/framework_lib.py`, `_framework/scripts/render_prompts.py`, `docs/sdd/SDD-DTF-0001.md`, `docs/sdd/registry.yaml` — todos previstos. Além desses, `_framework/prompts/{universal.md,cursor/doc-framework.mdc,copilot/copilot-instructions.md}` também aparecem no diff, mas só no bloco `<!-- BEGIN GENERATED -->` (bump `2.0.0` → `2.1.0` e remoção das duas linhas de PRD/TS da tabela) — é a saída determinística e pré-existente de `build_block`, a mesma função que a SDD instrui explicitamente a não reescrever, reagindo ao YAML mudado. Não é edição manual nem escopo fora do previsto; a SDD só proíbe alterar a *lógica* de `prompts/cursor`/`prompts/copilot` (etapa 3), não impede que o bloco gerado dessas renderizações mude quando o YAML muda.

## Descompassos encontrados (SDD-DTF-0001)

Nenhum quanto ao escopo desta SDD. A tabela de evidência original (2026-08-29) já registrava exit 0/sim em todos os 10 critérios; esta verificação independente reproduz o mesmo resultado 17 dias depois e por cima de dezenas de commits subsequentes, confirmando que as garantias da etapa 0 não regrediram.

O critério 9 (paridade entre repositórios) merece uma nota: `diff -r` completo entre os dois `_framework/` diverge amplamente hoje, mas por SDDs posteriores aplicadas só em `doc-traceability-framework` e ainda não replicadas para `doc-traceability-central` — não por causa do que esta SDD introduziu. Os dois artefatos que esta SDD de fato entrega (`AGENTS.md`, `QUICKSTART.md`) seguem idênticos entre os repositórios.

## Lições (SDD-DTF-0001)

- Uma SDD aprovada e mergeada pode ficar meses sem virar `implemented` porque o gate de verificação independente foi criado depois dela — vale reverificar critérios de aceite ainda válidos direto no `HEAD` atual, não só no diff do PR original, quando o código evoluiu por cima.
- Bloco gerado (`<!-- BEGIN GENERATED -->`) que muda em arquivos fora da lista de "Arquivos a alterar" da SDD não é necessariamente scope creep: se o gerador já existia e só está reagindo a uma entrada que a própria SDD mudou (o YAML), é efeito esperado, não código extra — a leitura de "arquivo fora da lista" precisa diferenciar "lógica alterada" de "saída determinística regerada".
