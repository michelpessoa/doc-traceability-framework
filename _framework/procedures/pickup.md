...
6. **Prossiga.** Execute o "Next step" diretamente — quem chamou o
   procedimento já decidiu continuar, não pergunte "posso continuar?". Só
   pare para perguntar se o "Next step" for genuinamente ambíguo, e aí
...
2. **Gates continuam valendo.** Retomar via este procedimento não pula
   nenhum gate do framework — se o "Next step" é implementar e a SDD não
   está `approved`, ou se o commit ainda não tem branch dedicada, aplique
   o gate correspondente (seções 13/14/16 de `workflow-rules.yaml`) antes
   de agir.
3. **Não apague o HANDOFF.** Quem decide quando o HANDOFF.md some é o
   usuário, não este procedimento.
