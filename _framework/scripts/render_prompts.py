...
from render_indexes import build_toc, generate_all, inject_toc  # noqa: E402
...
def build_copilot_instructions(rules: dict) -> str:
    ...
    Instruções para .github/copilot-instructions.md do repositório de
    projeto.
    """
    fw = rules.get("framework") or {}
    version = fw.get("version")
...
def build_claude_agent(capability: dict, rules: dict) -> str:
    ...
    if procedure:
        body = (
            f"Segue integralmente o procedimento normativo em "
            f"`_framework/{procedure}` (capacidade `{capability['id']}`)."
        )
    else:
...
"""
    lines = [header, "while IFS= read -r segment; do", '  case "$segment" in']
    for entry in patterns:
        lines.append(f"    {entry['pattern']})")
        lines.append(f'      deny "{entry["message"]}" ;;')
    lines.append("  esac")
    lines.append('done <<<"$segments"')
    lines.append("")
    lines.append("exit 0")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    ok &= generate_all(root, load_rules(), check)
